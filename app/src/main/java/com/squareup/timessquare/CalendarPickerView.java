...
import static com.makent.trioangle.helper.Constants.isCheckAvailability;
...
public class CalendarPickerView extends ListView {
    ...
    String samemonth = null;

    // For Makent Space:
    String checkinouttime=null;
    ...
    public CalendarPickerView(Context context, AttributeSet attrs) {
        ...
        dayTextColorResId = a.getResourceId(R.styleable.CalendarPickerView_tsquare_dayTextColor, R.color.calendar_text_selector);
        titleTextColor = a.getColor(R.styleable.CalendarPickerView_tsquare_titleTextColor, res.getColor(R.color.calendar_text_active));
        displayHeader = a.getBoolean(R.styleable.CalendarPickerView_tsquare_displayHeader, true);
        headerTextColor = a.getColor(R.styleable.CalendarPickerView_tsquare_headerTextColor, res.getColor(R.color.calendar_text_active));
        a.recycle();
        ...
        if(getPreferedlanguageCode.equals(Constants.ARABIC)){
            locale = new Locale(Constants.ARABIC,"SA");
        }else {
    ...
    public FluentInitializer init(Date minDate, Date maxDate, TimeZone timeZone) {
        ...
        return init(minDate, maxDate, timeZone, locale);
    }
        ...
        public void handleClick(MonthCellDescriptor cell) {
            ...
            if (!betweenDates(clickedDate, minCal, maxCal) || !isDateSelectable(clickedDate)) {
                if (invalidDateListener != null) {
                    invalidDateListener.onInvalidDateSelected(clickedDate,getResources().getString(R.string.datesnotavailable));
                }
            ...
            }/*else if(clickedDate.after(maxCal.getTime())){
                if (invalidDateListener != null) {
                    invalidDateListener.onInvalidDateSelected(clickedDate,getResources().getString(R.string.pleasepickpreviousdate));
                }
            }*/
            else {
    ...
    private void validateDate(Date date) {
        ...
        {
           snackBar(getResources().getString(R.string.pleasepickpreviousdate));
        }
    }
    private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
        Calendar newlySelectedCal = Calendar.getInstance(timeZone, locale);
        newlySelectedCal.setTime(date);
        System.out.print("newSelectedCal"+newlySelectedCal);
        ...
        if (date != null) {
            ...
            if (selectedCells.size() == 0 || !selectedCells.get(0).equals(cell)) {
                ...
                if (selectedCells.size() == 1) {
                    ...
                    c.setTime(selectedCells.get(0).getDate());
                    c.add(Calendar.DATE, 0);
                    Date newDate = c.getTime();
                    if(newDate.after(maxCal.getTime()))
                    {
                        if(invalidDateListener!=null)
                        {
                            invalidDateListener.onInvalidDateSelected(newDate,getResources().getString(R.string.pleasepickpreviousdate));
                        }
                        clearOldSelections();
                        return false;
                    }
                    ...
                    {
                        String enddates = newDate.toString();
    ...
    private class MonthAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (monthView == null || !monthView.getTag(R.id.day_view_adapter_class).equals(dayViewAdapter.getClass())) {
                monthView = MonthView.create(parent, inflater, weekdayNameFormat, listener, today, dividerColor, dayBackgroundResId, dayTextColorResId, titleTextColor, displayHeader, headerTextColor, decorators, locale, dayViewAdapter);
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
        ...
        while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
            ...
            for (int c = 0; c < 7; c++) {
                ...
                weekCells.add(new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected, isToday, isHighlighted, isBlocked, value, rangeState));
                cal.add(DATE, 1);
            }
        }
        return cells;
    ...
    public interface OnInvalidDateSelectedListener {
        void onInvalidDateSelected(Date date,String msg);
    }
    ...
    public String getFormatedDate(String strDate) {
        System.out.println(" Date Formated Date " + strDate);
        ...
        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.KITKAT) {
            ...
            if (!getPreferedlanguageCode.equalsIgnoreCase("en")){
                ...
                spdate = readFormat.format(dates);
                System.out.println("get in Arabic "+spdate);
        ...
        System.out.println(" Date Formated Date1 " + formattedDate);
        StringTokenizer tokens = new StringTokenizer(formattedDate, " ");
        System.out.println(" Date Formated tokens " + tokens);
        ...
        String dateformated = "" + first + "\n" + second + " " + third;
        if (checkinout != null) {
            System.out.println("Same Month" + samemonth + " This month" + second);
            if (samemonth.equals(second)) {
                checkinout = checkinout + "-" + third;
                //For makent space:
                System.out.println("checkinouttime : "+checkinouttime+" starttime "+starttime+endtime);
                checkinouttime= checkinouttime+" ("+starttime+") - "+third+" ("+endtime+")";

            } else {
                checkinout = checkinout + "-" + second + " " + third;
                //For makent space:
                checkinouttime= checkinouttime+" ("+starttime+") - "+second+" "+third+" ("+endtime+")";
            }
            System.out.println("Checkinout check " + checkinout);
        } else {

            samemonth = second;
            checkinout = second + " " + third;

            //For Makent Space:
            checkinouttime=second + " " + third;

            System.out.println("Checkinout checkings " + checkinout + " samemonth " + samemonth + " checkinout " + second + " " + third);

        }
        // calendarActivity.cal_chin.setText("" + first + "\n" + second + " " + third);

        System.out.println("DateFormat"+dateformated);

        System.out.println("get Final Formatted datess "+dateformated);
        return dateformated;
    }

    public  String getFormatedDateSearch(String strDate) {
        String dateStr = strDate;
        DateFormat readFormat;
        DateFormat writeFormat;
        DateFormat writeFormatEn = new SimpleDateFormat("dd-MM-yyyy", Locale.ENGLISH);
        String spdate="";

        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.KITKAT) {
            if (!getPreferedlanguageCode.equalsIgnoreCase("en")){
                readFormat = new SimpleDateFormat("EEE MMM dd hh:mm:ss zzz yyy", new Locale(getPreferedlanguageCode));
                writeFormat = new SimpleDateFormat("dd-MM-yyyy", new Locale(getPreferedlanguageCode));
                writeFormatEn = new SimpleDateFormat("dd-MM-yyyy", Locale.ENGLISH);
                ...
                readFormat = new SimpleDateFormat("EEE MMM dd hh:mm:ss zzz yyy", Locale.getDefault());
                writeFormat = new SimpleDateFormat("dd-MM-yyyy", Locale.getDefault());
                spdate=strDate;
            }
        }else{
            if (!getPreferedlanguageCode.equalsIgnoreCase("en")){
                ...
                readFormat = new SimpleDateFormat("EEE MMM dd hh:mm:ss zzz yyy", Locale.ENGLISH);
                writeFormat = new SimpleDateFormat("dd-MM-yyyy", Locale.ENGLISH);
                spdate=strDate;
            }
        }
        ...
        if (date != null) {
            formattedDate = writeFormat.format(date);
            formattedDates = writeFormatEn.format(date);
        }
        ...
        return dateformated+","+formattedDates;
    }

    public void startdate(String stdate) {
        System.out.print("stdate"+stdate);
        String startdate = getFormatedDate(stdate);
        String[] getsearchstartdate = getFormatedDateSearch(stdate).split(",");
        String searchstartdate="";
        if ("0".equalsIgnoreCase(getResources().getString(R.string.layout_direction))) {
            searchstartdate = getsearchstartdate[0];
        } else {
            searchstartdate = getsearchstartdate[1];
        }

        localSharedPreferences.saveSharedPreferences(Constants.CheckIn, searchstartdate);
        localSharedPreferences.saveSharedPreferences(Constants.CheckInDetail, startdate);

        System.out.println("checkinout startdate " + checkinout + " Startdate " + startdate + " searchstartdate " + searchstartdate);

        calendarActivity.cal_chin.setText(startdate);
        if (isCheckAvailability.equals("0"))
            calendarActivity.rltPickDrop.setVisibility(VISIBLE);
    }


    public void enddate(String endate) {

        System.out.print("endate"+endate);
        String enddate = getFormatedDate(endate);
        String[] getsearchenddate = getFormatedDateSearch(endate).split(",");
        String searchenddate="";
        if ("0".equalsIgnoreCase(getResources().getString(R.string.layout_direction))) {
            searchenddate = getsearchenddate[0];
        } else {
            searchenddate = getsearchenddate[1];
        }
        //if(highlightedCals==null) {
        System.out.println("checkinout enddate " + checkinout + " searchenddate " + searchenddate + " enddate " + enddate);
        localSharedPreferences.saveSharedPreferences(Constants.CheckOut, searchenddate);
        localSharedPreferences.saveSharedPreferences(Constants.CheckOutDetail, enddate);
        localSharedPreferences.saveSharedPreferences(Constants.CheckInOut, checkinout);
        //For makent space:
        localSharedPreferences.saveSharedPreferences(Constants.CheckInOuttime, checkinouttime);
        //}
        calendarActivity.cal_chout.setText(enddate);
        samemonth = null;
        checkinout = null;
    }
    ...
    private class DefaultOnInvalidDateSelectedListener implements OnInvalidDateSelectedListener {
        @Override
        public void onInvalidDateSelected(Date date,String msg) {
      ...
      /*Snackbar snackbar = Snackbar
              .make(cal_chin, "Those dates are not available...", Snackbar.LENGTH_LONG);
    ...
    public void snackBar(String msg) {
        // Create the Snackbar
        Snackbar snackbar = Snackbar.make(calendarActivity.cal_chin, "", Snackbar.LENGTH_LONG);
        ...
        textViewTop.setTextColor(getResources().getColor(R.color.background));
